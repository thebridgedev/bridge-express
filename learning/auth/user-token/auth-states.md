...
A frontend Bridge SDK tracks a single reactive `authState` that walks a signed-in user through a multi-step sign-in flow (credentials, MFA, workspace selection, fully authenticated). An Express backend doesn't have a sign-in flow to walk through; it evaluates one request at a time, independently, with no memory of the previous one. The useful equivalent here is the set of outcomes `bridge.auth()` / `bridge.protect(...)` can land a request in.

## The outcomes

| Outcome | HTTP result | What's on `req` afterward |
|---|---|---|
| No credential offered | `401` with `WWW-Authenticate: ... error="missing_token"` | Nothing; handler never runs |
