...
Your Express app has no way to force that refresh from the server side; there's no revocation list for user JWTs (see [Signing in and signing out](/auth/user-token/logging-in-and-out/)). If you need a role/privilege change to take effect *immediately*, regardless of what token a caller is still holding, put the check behind something the server does control (an API-token privilege, which is instantly revocable via introspection, or your own database lookup) rather than relying solely on JWT claims.
...
Right after a mutation that affects this data (you just upgraded the workspace's plan, say; a workspace is called a *tenant* in the API), call `invalidate()` so the next read is forced fresh instead of serving the stale cached snapshot:
