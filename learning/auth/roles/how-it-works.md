...
Notably, the backend sees **more** than a typical frontend does here: a frontend's live snapshot exposes `role` but not the underlying `privileges` array (privileges travel in the JWT, not the frontend session snapshot). Express verifies the JWT itself, so `req.bridgeUser.privileges` is available directly — useful if you need finer-grained checks than "does this role match."
...
admin.get('/settings', bridge.protect({ role: 'OWNER' }), (req, res) => {
  // route-level middleware — requires OWNER in addition to the router-level ADMIN check
