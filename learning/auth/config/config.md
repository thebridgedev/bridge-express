...
| `guard` | `GuardConfig` | — | Declarative route rules + default access — see [Route guards](/auth/securing/route-guards/) |
...
| `introspectionCacheTtlMs` | `number` | `0` | How long (ms) a successful API-token introspection is cached, keyed by token. `0` disables caching → every request introspects (instant revocation); raise it to trade revocation latency for fewer network calls. |
...
| Unified tenant surface (`bridge.fromJwt()`) | `{apiBaseUrl}/session/init` | — |
...
Some settings aren't passed to `createBridge(...)` at all — they're set once per app in Control Center (or via the CLI), and Bridge enforces them server-side regardless of which SDK is talking to it:

| Setting | What it does |
|---------|---------------|
| Redirect URIs | The allowlist of callback URLs Bridge is allowed to redirect a frontend login flow to. |
...
| SSO providers | Federation connections (Google, Azure AD, etc.) available to the login flow. |
